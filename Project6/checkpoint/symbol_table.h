...
protected:
  ...
  // If this variable is an array index, we need more information about it.
  tableEntry * array_ptr;
  tableEntry * index_ptr;
...
public:
  ...
  tableEntry * GetIndexPtr() { return index_ptr; }
  int GetArrayID() const     { return array_ptr->GetVarID(); }
  int GetIndexID() const     { return index_ptr->GetVarID(); }

  bool IsArrayIndex() const { return array_ptr != NULL; }
  ...
  void SetArrayIndex(tableEntry * a_ptr, tableEntry * i_ptr) {
    array_ptr = a_ptr; index_ptr = i_ptr;
  }
};
...
public:
  ...
  void RemoveEntry(tableEntry * del_var) {
    // If del_var is an array index, check the internal values as well.
    if (del_var->array_ptr && del_var->array_ptr->GetTemp()) RemoveEntry( del_var->array_ptr );
    if (del_var->index_ptr && del_var->index_ptr->GetTemp()) RemoveEntry( del_var->index_ptr );
