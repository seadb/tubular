...
%token CASSIGN_ADD CASSIGN_SUB CASSIGN_MULT CASSIGN_DIV CASSIGN_MOD COMP_EQU COMP_NEQU COMP_LESS COMP_LTE COMP_GTR COMP_GTE BOOL_AND BOOL_OR COMMAND_PRINT COMMAND_IF COMMAND_ELSE COMMAND_WHILE COMMAND_BREAK COMMAND_RANDOM
%token <lexeme> INT_LIT CHAR_LIT STRING_LIT ID TYPE META_TYPE
...
%type <ast_node> var_declare expression declare_assign statement statement_list var_usage array_index lhs_ok command argument_list code_block if_start while_start flow_command
%%
...
program:      statement_list {
                 ...
                 if (produce_IC) {
                   ...
                   ic_array.PrintTubeCode(out_file);            // Write Tubecode Assembly to output file!
                 }
              }
	     ;
...
var_declare:	TYPE ID {
		  ...
		  if (type_name == "int") type_id = Type::INT_ARRAY;
		    ...
		    std::stringstream err_string;
                    err_string << "unknown type '" << $1 << "(" << $3 << ")'";
		    yyerror(err_string.str());
		    exit(1);
		  }
...
expression:  expression '+' expression { 
	...
	|    CHAR_LIT {
        ...
        |    STRING_LIT {
               $$ = new ASTNode_Literal(Type::CHAR_ARRAY, $1);
               $$->SetLineNum(line_num);
             }
	|    var_usage { $$ = $1; }
	|    array_index { $$ = $1; }
