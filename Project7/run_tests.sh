...
function run_error_test {
    ...
    if [ -e ref.tca ] ; then
	    Test_Suite/tubecode -t 1000000 $project.tca > $project.out
	    Test_Suite/tubecode -t 1000000 ref.tca > ref.out
...
for F in Test_Suite/good*.tube.6; do 
	run_error_test $F "0"
done
for F in Test_Suite/fail*.tube.6; do 
	run_error_test $F "0"
done
