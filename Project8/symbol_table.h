...
public:
  ...
  bool GetDefined() const { return mDefined; }
  ...
  void SetDefined(bool in)               { mDefined = in;}
  //void SetSize(int inSize)             { mSize = inSize; }
  //void SetNegative(bool inNegative)    { mNegative = inNegative; }
};
...
private:
  ...
  std::map<std::string, CFunctionEntry *> mDeclaredFunctionMap;
  std::map<std::string, CFunctionEntry *> mDefinedFunctionMap;
...
public:
  ...
  void TransferArgs(CFunctionEntry * from, CFunctionEntry * to)
  {
    for(int i=0; i<from->GetNumArgs(); i++)
    {
        to->PushArg(from->GetArg(i));
    }
  ...
  ASTNode * GetFunctions() { return mFunctions; }
  ...
  CFunctionEntry * DeclaredFunctionLookup(std::string inName)
  {
    if(mDeclaredFunctionMap.find(inName) == mDeclaredFunctionMap.end()) return NULL;
    return mDeclaredFunctionMap[inName];
  }

  // Detemine if a function has already been defined
  CFunctionEntry * DefinedFunctionLookup(std::string inName)
  {
    if(mDefinedFunctionMap.find(inName) == mDefinedFunctionMap.end()) return NULL;
    return mDefinedFunctionMap[inName];
  }
  ...
  CTableEntry * AddEntry(int inType, std::string inName)
  {
    ...
    if(mFunctionMode) {
      ...
      if(!mCurrentFunction->GetDefined())
      {
  ...
  CFunctionEntry * AddFunctionTemp(int inType)
  {
    CFunctionEntry * newEntry = new CFunctionEntry(inType);
    newEntry->SetVarID( GetNextID() );
    return newEntry;
  }

  // Insert a function into the symbol table
  CFunctionEntry * AddFunctionDefine(std::string name, int type)
  {
    CFunctionEntry * newEntry = new CFunctionEntry(name, type);
    newEntry->SetVarID( GetNextID());
    mDefinedFunctionMap[name] = newEntry;
    newEntry->SetReturn(AddTempEntry(type));
    newEntry->SetReturnValue(AddTempEntry(type));
    return newEntry;
  }

  CFunctionEntry * AddFunctionDeclare(std::string name, int type)
  {
    CFunctionEntry * newEntry = new CFunctionEntry(name,type);
    newEntry->SetVarID( GetNextID());
    mDeclaredFunctionMap[name] = newEntry;
    newEntry->SetReturn(AddTempEntry(type));
    newEntry->SetReturnValue(AddTempEntry(type));
    return newEntry;
  ...
  std::string CheckFunctions() {
    for (std::map<std::string, CFunctionEntry *>::iterator it=mDefinedFunctionMap.begin();
                                                      it!=mDefinedFunctionMap.end(); ++it)
    {
      CFunctionEntry * entry = mDefinedFunctionMap[it->first];
      if(!entry->GetDefined()) //GetDefined == false
      {
         std::string declaredName = entry->GetName();
         return declaredName;
      }
        //if(mDefinedFunctionMap.find(declaredName) == mDefinedFunctionMap.end()){
          // Declared Function not defined
         // return declaredName;
        //}
    }
    return ""; //all functions were defined
  }
};
