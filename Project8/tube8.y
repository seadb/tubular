...
COMMAND_RANDOM COMMAND_DEFINE COMMAND_RETURN SIZE RESIZE COMMAND_DECLARE
...
function_arguments args_begin args_end function_define function_call function_define_name//
...
program:      statement_list {
                 ...
                 //ic_array.PrintIC(out_file);
                 ic_array.OptimizeIC();
                 //std::cout << "statement_list" << std::endl;
                 ic_array.static_memory_size = symbol_table.GetTempVarID();
                 std::string function = symbol_table.CheckFunctions();
                 if(function != "")
                 {
                    std::string errString = "function declared but not defined: ";
                    errString += function;
                    yyerror(errString);
                    exit(1);
                 }
...
statement_list:   {
  ...
  |
          statement_list function_define {
...
function_call: ID '(' argument_list ')' {
                CFunctionEntry * defined = symbol_table.DefinedFunctionLookup($1);
                CFunctionEntry * declared = symbol_table.DeclaredFunctionLookup($1);
                CFunctionEntry * cur_entry;
                if (defined == NULL && declared == NULL)
                {
               ...
               else
                {
                  ...
                  std::stringstream ss;
                  ss << cur_entry->GetNumArgs();
                  //std::cout << ss.str() << std::endl;
                  if(defined->GetNumArgs() != $3->GetNumChildren())
                  {
                ...
                $$ = new ASTNodeFunctionCall(defined, $1);
                $$->SetType(defined->GetReturnType());
                $$->TransferChildren($3);
             }
           | ID '(' ')' {
                CFunctionEntry * defined = symbol_table.DefinedFunctionLookup($1);
...
function_declare:
                function_declare_name function_arguments {
...
function_declare_name:
                     COMMAND_DECLARE TYPE ID
                  {
                  if(symbol_table.DeclaredFunctionLookup($3) != 0)
                  //if(symbol_table.DefinedFunctionLookup($3) != 0)
                  {
                    std::string errString = "redeclaration of function '";
                    errString += $2;
                    errString += "'";
                    yyerror(errString);
                    exit(1);
                   ...
                   std::string name = $3;
                   CFunctionEntry * function = symbol_table.AddFunctionDeclare(name, type_id);
                   CFunctionEntry * defined = symbol_table.AddFunctionDefine(name, type_id);
                   symbol_table.SetCurrentFunction(defined);
...
function_define:
                function_define_name function_arguments code_block{
                  CFunctionEntry * temp = symbol_table.GetCurrentFunction();
                  CFunctionEntry * declared = symbol_table.DeclaredFunctionLookup(temp->GetName());
                  CFunctionEntry * defined = symbol_table.DefinedFunctionLookup(declared->GetName());
                  symbol_table.DecScope();

                  // check arguments of previously declared function

                    if(temp->GetNumArgs() != declared->GetNumArgs())
                    {
                      std::string errString = "defined number of arguments does not match declared number of arguments.";
                      errString += "\nDeclared number: ";
                      std::stringstream ss; ss << declared->GetNumArgs();
                      errString += ss.str();
                      errString += "\nDefined number: ";
                      ss.str(""); ss << temp->GetNumArgs();
                      errString += ss.str();
                      yyerror(errString);
                      exit(1);
                    }
                    if(declared->GetNumArgs() > 0)
                    {
                        for(int i = 0; i < declared->GetNumArgs(); i++)
                        {
                          if(temp->GetArg(i)->GetType() != declared->GetArg(i)->GetType() )
                          { // types do not match
                            std::string errString = "defined function argument does not match declared type\n";
                            errString += "Declared type: ";
                            errString += Type::AsString(declared->GetArg(i)->GetType());
                            errString += "\nDefined type: ";
                            errString += Type::AsString(temp->GetArg(i)->GetType());
                            yyerror(errString);
                            exit(1);
                            }
                        }
                    ...
                    defined->SetBody($3);
                    defined->SetDefined(true);
                    $$ = new ASTNodeFunction(defined, $3);
                    $$->TransferChildren($2);
                    $$->SetLineNum(line_num);

                }
               | function_define_name function_arguments command ';' {
                  CFunctionEntry * function= symbol_table.GetCurrentFunction();
                  function->SetBody($3);
                  symbol_table.DecScope();
...
function_define_name:
               COMMAND_DEFINE TYPE ID {
                  CFunctionEntry * declared = symbol_table.DeclaredFunctionLookup($3);
                  CFunctionEntry * defined = symbol_table.DefinedFunctionLookup($3);

                  if(defined != 0 && defined->GetDefined() == true)
                  {
                    std::string errString = "redefinition of function '";
                    errString += $3;
                    errString += "'";
                    yyerror(errString);
                    exit(1);
                  ...
                  if(declared !=0)
                  {
                    symbol_table.TransferArgs(defined, declared);
                    defined->ClearArgs();
                  }
                  if(defined != 0)
                  { //declared but not defined
                    ...
                    //Check return types
                    if(defined->GetReturnType() != type_id)
                    {
                      std::string errString = "definition of '";
                      errString += $3;
                      errString += "' does not match previous declaration";
                    }
                  }

                  std::string name = $3;
                  CFunctionEntry * temp = symbol_table.AddFunctionTemp(type_id);
                  temp->SetName(name);
                  symbol_table.SetCurrentFunction(defined);
    ...
    |        COMMAND_DEFINE ARRAY '(' TYPE ')' ID {
                  CFunctionEntry * defined = symbol_table.DefinedFunctionLookup($6);
                  if(defined != 0 && defined->GetDefined() == true)
                  {
                    std::string errString = "redefinition of function '";
                    errString += $6;
                  ...
                  if(symbol_table.DeclaredFunctionLookup($6) != 0)
                  {
                    //TODO: Check types
                    CFunctionEntry * defined = symbol_table.DeclaredFunctionLookup($6);
                    if(defined->GetReturnType() != type_id)
                    {
                      std::string errString = "function definition return type does not match declaration type: ";
                      errString += defined->GetName();
                      yyerror(errString);
                      exit(1);
                  ...
                  CFunctionEntry * function = symbol_table.AddFunctionDefine(name, type_id);
                  symbol_table.SetCurrentFunction(function);
